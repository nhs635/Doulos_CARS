...
namespace bf {
    ...
    inline void LinearInterp_32fc(const Ipp32f* src, Ipp32f* dst, int dst_len, Ipp32f* cal_ind, Ipp32f* cal_weight)
    {
        printf("%d\n", dst_len);
		for (int i = 0; i < dst_len; i++)
		{
                        dst[i] = src[i] ; //cal_weight[i] * src[(int)cal_ind[i]] + (1 - cal_weight[i]) * src[(int)cal_ind[i] + 1];
