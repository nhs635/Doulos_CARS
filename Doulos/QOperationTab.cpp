...
void QOperationTab::operateDataAcquisition(bool toggled)
{
    if (toggled) // Start Data Acquisition
    {
        if (m_pDataAcquisition->InitializeAcquistion())
        {
            ...
            m_pStreamTab->m_pThreadDataProcess->startThreading();
            ...
            if (m_pDataAcquisition->StartAcquisition())
            {
				...
				// Start Galvo-scan & Resonant-scan
                ...
                m_pStreamTab->getDeviceControlTab()->getZaberStageControl()->setChecked(true);

				Sleep(400);
				m_pStreamTab->getDeviceControlTab()->getResonantScanVoltageSpinBox()->setValue(RESONANT_DEFAULT_VOLTAGE);
			}
			else
			{
    ...
    else // Stop Data Acquisition
    {
        ...
        m_pStreamTab->m_pThreadDataProcess->stopThreading();
		...
		// Stop Galvo-scan & Resonant-scan
//        m_pStreamTab->getDeviceControlTab()->getPmtGainControl()->setChecked(false);
        m_pStreamTab->getDeviceControlTab()->getZaberStageControl()->setChecked(false);
...
void QOperationTab::operateDataRecording(bool toggled)
{
    if (toggled) // Start Data Recording
    {
		if (m_pMemoryBuffer->startRecording())
		{
            ...
            m_pStreamTab->getDeviceControlTab()->setRelevantWidgets(false);
            m_pStreamTab->getVisualizationTab()->setRelevantWidgets(false);
		}
		else
		{
...
#else
        m_pToggleButton_Saving->setEnabled(m_pMemoryBuffer->m_nRecordedImages != 0);
#endif
		m_pStreamTab->setAveragingWidgets(true);        
        m_pStreamTab->getDeviceControlTab()->setRelevantWidgets(true);
        m_pStreamTab->getVisualizationTab()->setRelevantWidgets(true);

#ifdef RAW_PULSE_WRITE
		if (m_pMemoryBuffer->m_nRecordedFrames > 1)
			m_pProgressBar->setRange(0, m_pMemoryBuffer->m_nRecordedFrames - 1);
#else
        if (m_pMemoryBuffer->m_nRecordedImages > 1)
            m_pProgressBar->setRange(0, m_pMemoryBuffer->m_nRecordedImages - 1);
#endif
		else
